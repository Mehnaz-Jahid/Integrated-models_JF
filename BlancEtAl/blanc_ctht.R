...
y <- read.csv ('CTcapthist.csv') # load presence-absence data
nsites <- dim(y)[1]
nsurvs <- dim(y)[2]
...
for (i in 1:nsites){
    ...
    logit(psi[i]) <- mupsi + betapsi[i] # occupancy probability as a function of ???? and a site random effect ??s
 ...
 for(i in 1:M){
    ...
    N ~ dpois(lamn)T(0,M)
    psi0 <- 1/(1+exp(-mupsi))  # ???? back transformed
    log(lamn) <- log(-log(1-psi0)) # definition of ?? as a function of ??
    mumup0 <- 1/(1+exp(-mumup)) # ??p back transformed
    zerouse ~ dpois(comb)
